/**
* @project_name Sofia [WA Multi-device]
...
AMDI({ cmd: ["اغنية", "yta", "mp3"], desc: Lang.songDesc, example: Lang.songExa, type: "download", react: "🎵" }, (async (amdiWA) => {
    ...
    if (!execYT.isYT) {
        ...
        const ytDlTXT = `*🎶 Sofia YT Downloader*\n\n📄 ${Lang.Title} ${title}`
        const buttons = [
...
AMDI({ cmd: ["video", "ytv", "فيديو"], desc: Lang.videoDesc, example: Lang.videoExa, type: "download", react: "🎞️" }, (async (amdiWA) => {
    let { input, prefix, reply, sendButtonsMsg, sendListMsg, urlRegexYT } = amdiWA.msgLayout;
    ...
    if (!execYT.isYT) {
        ...
        const title = ytVidList[0] ? ytVidList[0].title : ''
        const ytDlTXT = `*🎞️ Sofia YT Downloader*\n\n📄 ${Lang.Title} ${title}`
        const buttons = [
            { type: "url", displayText: "Watch on YouTube", url: input },
...
AMDI({ cmd: "تحميل من اليوتيوب", cmdHideInMenu: true, type: "download" }, (async (amdiWA) => {
